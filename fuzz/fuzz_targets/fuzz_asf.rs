//! Fuzz target for the ASF (WMA/WMV) format parser and writer.
//!
//! Exercises tag read/write, multi-value tags, tag removal, and clear-before-save
//! to cover the full mutation surface for ASF files.
...
fuzz_target!(|data: &[u8]| {
    ...
    let Some((_dir, path)) = helpers::write_temp_file(data, "wma") else {
        return;
    };
    let Ok(mut f) = ASF::load(&path) else {
        return;
    };
    ...
    let _ = f.set("tracknumber", vec!["2".into()]);
