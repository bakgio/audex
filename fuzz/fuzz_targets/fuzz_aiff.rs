//! Fuzz target for the AIFF format parser and writer.
//!
//! Exercises tag read/write, multi-value tags, tag removal, and clear-before-save
//! to cover the full mutation surface for AIFF files.
...
fuzz_target!(|data: &[u8]| {
    ...
    let Some((_dir, path)) = helpers::write_temp_file(data, "aif") else {
        return;
    };
    let Ok(mut f) = AIFF::load(&path) else {
        return;
    };
    ...
    let _ = f.set("tracknumber", vec!["3".into()]);

    // Save and reload to verify round-trip integrity
    if f.save().is_ok() {
        let _ = AIFF::load(&path);
    }
